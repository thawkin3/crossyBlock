(function() {
    var gameController = function ($scope, $routeParams, $rootScope, $location, $timeout, $interval) {
        ...
        $scope.submitScore = function() {
            ...
            // there is a leeway of 5 points since it's possible you could have moved backwards
            var calculatedScore;
            var scoreToSubmit = $scope.score;
            for (var i = obstacles.length - 1; i >= 0; i--) {
                if (obstacles[i][0].top > player_y) {
                    calculatedScore = i + 1;
                    break;
                }
            }
            if ($scope.score > calculatedScore && $scope.score - calculatedScore > 5) {
                scoreToSubmit = calculatedScore;
            }

            var scoreObj = { Username: $rootScope.root.username, Score: scoreToSubmit };
